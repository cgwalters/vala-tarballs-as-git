...
static void vala_gerror_module_real_return_with_exception (ValaGErrorModule* self, ValaCCodeExpression* error_expr) {
	...
	if (_tmp21_) {
		...
		_tmp25_ = _tmp24_;
		_tmp26_ = _vala_code_node_ref0 (VALA_CLASS (_tmp25_));
		cl = _tmp26_;
...
static void vala_gerror_module_uncaught_error_statement (ValaGErrorModule* self, ValaCCodeExpression* inner_error, gboolean unexpected) {
	...
	if (_tmp53_) {
		...
		_tmp55_ = _tmp54_;
		if (VALA_IS_CREATION_METHOD (_tmp55_)) {
			ValaMethod* _tmp56_;
			ValaMethod* _tmp57_;
			ValaSymbol* _tmp58_;
			ValaSymbol* _tmp59_;
			...
			handled_error_types = _tmp38_;
			{
				...
				while (TRUE) {
					...
					if (_tmp92_) {
						...
						_tmp100_ = _tmp99_;
						_tmp101_ = _vala_code_node_ref0 (VALA_IS_ERROR_TYPE (_tmp100_) ? ((ValaErrorType*) _tmp100_) : NULL);
						catch_type = _tmp101_;
						_tmp102_ = catch_type;
...
static void vala_gerror_module_real_visit_catch_clause (ValaCodeVisitor* base, ValaCatchClause* clause) {
	...
	_tmp2_ = _tmp1_;
	_tmp3_ = _vala_code_node_ref0 (VALA_ERROR_TYPE (_tmp2_));
	error_type = _tmp3_;
	_tmp4_ = error_type;
...
static void vala_gerror_module_real_append_scope_free (ValaCCodeBaseModule* base, ValaSymbol* sym, ValaCodeNode* stop_at) {
	...
	_tmp0_ = sym;
	_tmp1_ = stop_at;
	VALA_CCODE_BASE_MODULE_CLASS (vala_gerror_module_parent_class)->append_scope_free ((ValaCCodeBaseModule*) VALA_CCODE_DELEGATE_MODULE (self), _tmp0_, _tmp1_);
	_tmp3_ = stop_at;
	if (VALA_IS_TRY_STATEMENT (_tmp3_)) {
		...
		ValaCodeNode* _tmp4_;
		_tmp4_ = stop_at;
		_tmp2_ = VALA_IS_CATCH_CLAUSE (_tmp4_);
	}
	_tmp5_ = _tmp2_;
	if (!_tmp5_) {
		ValaBlock* _tmp6_;
		...
		if (VALA_IS_TRY_STATEMENT (_tmp9_)) {
			...
			_tmp12_ = _tmp11_;
			_tmp13_ = vala_try_statement_get_finally_body (VALA_IS_TRY_STATEMENT (_tmp12_) ? ((ValaTryStatement*) _tmp12_) : NULL);
			_tmp14_ = _tmp13_;
			...
			_tmp18_ = _tmp17_;
			if (VALA_IS_CATCH_CLAUSE (_tmp18_)) {
				ValaSymbol* _tmp19_;
				...
				_tmp23_ = _tmp22_;
				_tmp24_ = vala_try_statement_get_finally_body (VALA_IS_TRY_STATEMENT (_tmp23_) ? ((ValaTryStatement*) _tmp23_) : NULL);
				_tmp25_ = _tmp24_;
		...
		if (_tmp28_ != NULL) {
			...
			_tmp30_ = sym;
			_tmp27_ = VALA_SYMBOL (_tmp29_) != _tmp30_;
		} else {
			_tmp27_ = FALSE;
		}
		_tmp31_ = _tmp27_;
