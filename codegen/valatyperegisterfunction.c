...
void vala_typeregister_function_init_from_type (ValaTypeRegisterFunction* self, gboolean plugin, gboolean declaration_only) {
	...
	_tmp131_ = _tmp130_;
	_tmp132_ = VALA_IS_OBJECT_TYPE_SYMBOL (_tmp131_);
...
"VABLE | G_TYPE_FLAG_DEEP_DERIVABLE) }");
	...
	_tmp216_ = _tmp215_;
	_tmp217_ = VALA_IS_STRUCT (_tmp216_);
	...
	if (_tmp217_) {
		...
		_tmp222_ = _tmp221_;
		_tmp223_ = VALA_IS_ENUM (_tmp222_);
		...
		if (_tmp223_) {
			...
			en = VALA_IS_ENUM (_tmp224_) ? ((ValaEnum*) _tmp224_) : NULL;
			_tmp225_ = en;
	...
	_tmp273_ = _tmp272_;
	_tmp274_ = VALA_IS_STRUCT (_tmp273_);
	...
	if (_tmp274_) {
		...
		_tmp293_ = _tmp292_;
		_tmp294_ = VALA_IS_ENUM (_tmp293_);
		...
		if (_tmp294_) {
			...
			en = VALA_IS_ENUM (_tmp295_) ? ((ValaEnum*) _tmp295_) : NULL;
	...
	if (!_tmp422_) {
		...
		if (_tmp464_) {
			...
			_tmp470_ = cond;
			_tmp471_ = condition;
			vala_ccode_function_call_add_argument (VALA_IS_CCODE_FUNCTION_CALL (_tmp470_) ? ((ValaCCodeFunctionCall*) _tmp470_) : NULL, _tmp471_);
		}
		_tmp472_ = cond;
		_tmp473_ = type_init;
