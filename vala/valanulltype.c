...
static gboolean vala_null_type_real_compatible (ValaDataType* base, ValaDataType* target_type) {
	...
	_tmp9_ = target_type;
	if (!VALA_IS_POINTER_TYPE (_tmp9_)) {
		gboolean _tmp10_ = FALSE;
		ValaDataType* _tmp11_;
		gboolean _tmp20_;
		_tmp11_ = target_type;
		if (VALA_IS_NULL_TYPE (_tmp11_)) {
			_tmp10_ = TRUE;
		} else {
	...
	if (_tmp27_ != NULL) {
		...
		_tmp28_ = target_type;
		_tmp24_ = VALA_IS_POINTER_TYPE (_tmp28_);
	}
	_tmp29_ = _tmp24_;
	if (_tmp29_) {
		_tmp23_ = TRUE;
	...
	if (_tmp45_) {
		...
		_tmp46_ = target_type;
		_tmp41_ = VALA_IS_ARRAY_TYPE (_tmp46_);
	}
	_tmp47_ = _tmp41_;
	if (_tmp47_) {
		_tmp40_ = TRUE;
		...
		_tmp48_ = target_type;
		_tmp40_ = VALA_IS_DELEGATE_TYPE (_tmp48_);
	}
	_tmp49_ = _tmp40_;
	if (_tmp49_) {
		result = TRUE;
