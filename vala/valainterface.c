...
static void vala_interface_real_add_method (ValaSymbol* base, ValaMethod* m) {
	...
	if (!VALA_IS_VOID_TYPE (_tmp27_)) {
		...
		_tmp32_ = _tmp31_;
		_tmp33_ = _tmp32_ == VALA_PROFILE_DOVA;
	...
	if (_tmp40_) {
		...
		_tmp48_ = _tmp47_;
		_tmp49_ = vala_local_variable_new (_tmp46_, "result", NULL, _tmp48_);
		_tmp50_ = _tmp49_;
		vala_subroutine_set_result_var ((ValaSubroutine*) _tmp41_, _tmp50_);
		_vala_code_node_unref0 (_tmp50_);
		_vala_code_node_unref0 (_tmp46_);
		_tmp51_ = m;
		_tmp52_ = vala_subroutine_get_result_var ((ValaSubroutine*) _tmp51_);
		_tmp53_ = _tmp52_;
		vala_local_variable_set_is_result (_tmp53_, TRUE);
	}
	_tmp54_ = self->priv->methods;
	_tmp55_ = m;
	vala_collection_add ((ValaCollection*) _tmp54_, _tmp55_);
	_tmp56_ = vala_symbol_get_scope ((ValaSymbol*) self);
	_tmp57_ = _tmp56_;
	_tmp58_ = m;
	_tmp59_ = vala_symbol_get_name ((ValaSymbol*) _tmp58_);
	_tmp60_ = _tmp59_;
	_tmp61_ = m;
	vala_scope_add (_tmp57_, _tmp60_, (ValaSymbol*) _tmp61_);
}
...
static gboolean vala_interface_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	prereq_class = NULL;
	{
		...
		while (TRUE) {
			...
			if (!VALA_IS_OBJECT_TYPE_SYMBOL (_tmp67_)) {
...
"nd `%s')", _tmp82_, _tmp85_, _tmp88_);
				...
				_tmp92_ = _vala_code_node_ref0 (VALA_CLASS (_tmp91_));
				_vala_code_node_unref0 (prereq_class);
