...
static gboolean vala_object_type_real_stricter (ValaDataType* base, ValaDataType* target_type) {
	...
	_tmp0_ = target_type;
	_tmp1_ = _vala_code_node_ref0 (VALA_IS_OBJECT_TYPE (_tmp0_) ? ((ValaObjectType*) _tmp0_) : NULL);
...
static gboolean vala_object_type_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	if (_tmp6_ == VALA_PROFILE_DOVA) {
		...
		_tmp9_ = _tmp8_;
		_tmp3_ = g_strcmp0 (_tmp9_, "Dova.Tuple") == 0;
		_g_free0 (_tmp9_);
	} else {
		_tmp3_ = FALSE;
	}
	...
	if (_tmp24_) {
		...
		_tmp26_ = _tmp25_;
		vala_report_error (_tmp26_, "too few type arguments");
		result = FALSE;
		return result;
	} else {
		...
		if (_tmp28_ > 0) {
			...
			gint _tmp34_;
			_tmp29_ = n_type_args;
			_tmp30_ = self->priv->_type_symbol;
			_tmp31_ = vala_object_type_symbol_get_type_parameters (_tmp30_);
			_tmp32_ = _tmp31_;
			_tmp33_ = vala_collection_get_size ((ValaCollection*) _tmp32_);
			_tmp34_ = _tmp33_;
			_tmp27_ = _tmp29_ > _tmp34_;
		...
		if (_tmp35_) {
			...
			_tmp37_ = _tmp36_;
			vala_report_error (_tmp37_, "too many type arguments");
			result = FALSE;
			return result;
		}
	}
