...
static gboolean vala_addressof_expression_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	_tmp11_ = _tmp10_;
	_tmp12_ = _vala_code_node_ref0 (VALA_IS_ELEMENT_ACCESS (_tmp11_) ? ((ValaElementAccess*) _tmp11_) : NULL);
	ea = _tmp12_;
	...
	if (VALA_IS_MEMBER_ACCESS (_tmp15_)) {
		...
		_tmp19_ = _tmp18_;
		_tmp13_ = VALA_IS_VARIABLE (_tmp19_);
	} else {
		_tmp13_ = FALSE;
	}
	_tmp20_ = _tmp13_;
	if (_tmp20_) {
		...
		if (_tmp22_ != NULL) {
			...
			if (VALA_IS_ARRAY_TYPE (_tmp28_)) {
				...
				_tmp33_ = _tmp32_;
				_tmp23_ = VALA_IS_POINTER_TYPE (_tmp33_);
			}
			_tmp34_ = _tmp23_;
			_tmp21_ = _tmp34_;
		} else {
