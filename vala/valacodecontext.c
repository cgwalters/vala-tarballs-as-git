...
gboolean vala_code_context_add_source_filename (ValaCodeContext* self, const gchar* filename, gboolean is_source, gboolean cmdline) {
	...
	if (_tmp15_) {
		...
		_tmp21_ = self->priv->_profile;
		if (_tmp21_ == VALA_PROFILE_POSIX) {
			...
			_tmp22_ = vala_unresolved_symbol_new (NULL, "Posix", NULL);
			_tmp23_ = _tmp22_;
			_tmp24_ = vala_using_directive_new ((ValaSymbol*) _tmp23_, NULL);
			_tmp25_ = _tmp24_;
			_vala_code_node_unref0 (_tmp23_);
			ns_ref = _tmp25_;
			_tmp26_ = source_file;
			_tmp27_ = ns_ref;
			vala_source_file_add_using_directive (_tmp26_, _tmp27_);
			_tmp28_ = vala_code_context_get_root (self);
			_tmp29_ = _tmp28_;
			_tmp30_ = ns_ref;
			vala_namespace_add_using_directive (_tmp29_, _tmp30_);
			_vala_code_node_unref0 (ns_ref);
		} else {
			ValaProfile _tmp31_;
			_tmp31_ = self->priv->_profile;
			if (_tmp31_ == VALA_PROFILE_GOBJECT) {
				...
				_tmp38_ = vala_code_context_get_root (self);
				_tmp39_ = _tmp38_;
				_tmp40_ = ns_ref;
				...
				if (_tmp41_ == VALA_PROFILE_DOVA) {
					...
					_tmp42_ = vala_unresolved_symbol_new (NULL, "Dova", NULL);
					_tmp43_ = _tmp42_;
					_tmp44_ = vala_using_directive_new ((ValaSymbol*) _tmp43_, NULL);
					_tmp45_ = _tmp44_;
					_vala_code_node_unref0 (_tmp43_);
					ns_ref = _tmp45_;
					_tmp46_ = source_file;
					_tmp47_ = ns_ref;
					vala_source_file_add_using_directive (_tmp46_, _tmp47_);
					_tmp48_ = vala_code_context_get_root (self);
					_tmp49_ = _tmp48_;
					_tmp50_ = ns_ref;
					vala_namespace_add_using_directive (_tmp49_, _tmp50_);
					_vala_code_node_unref0 (ns_ref);
				}
			}
		}
		_tmp51_ = source_file;
		vala_code_context_add_source_file (self, _tmp51_);
		...
		if (_tmp54_) {
			...
			_tmp55_ = filename;
			_tmp56_ = g_str_has_suffix (_tmp55_, ".gir");
			_tmp52_ = _tmp56_;
		}
		_tmp57_ = _tmp52_;
		if (_tmp57_) {
			...
			_tmp58_ = rpath;
			_tmp59_ = cmdline;
			_tmp60_ = vala_source_file_new (self, VALA_SOURCE_FILE_TYPE_PACKAGE, _tmp58_, NULL, _tmp59_);
			source_file = _tmp60_;
			_tmp61_ = source_file;
			_tmp62_ = filename;
			vala_source_file_set_relative_filename (_tmp61_, _tmp62_);
			_tmp63_ = source_file;
			vala_code_context_add_source_file (self, _tmp63_);
			_vala_source_file_unref0 (source_file);
		} else {
			const gchar* _tmp64_;
			gboolean _tmp65_ = FALSE;
			_tmp64_ = filename;
			_tmp65_ = g_str_has_suffix (_tmp64_, ".c");
			if (_tmp65_) {
				const gchar* _tmp66_;
				_tmp66_ = rpath;
				vala_code_context_add_c_source_file (self, _tmp66_);
			} else {
				const gchar* _tmp67_;
				gchar* _tmp68_ = NULL;
				gchar* _tmp69_;
				_tmp67_ = filename;
				_tmp68_ = g_strdup_printf ("%s is not a supported source file type. Only .vala, .vapi, .gs, and .c" \
" files are supported.", _tmp67_);
				_tmp69_ = _tmp68_;
