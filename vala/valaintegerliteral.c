...
static gboolean vala_integer_literal_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	const gchar* _tmp69_;
	const gchar* _tmp70_;
	ValaIntegerType* _tmp71_;
	ValaIntegerType* _tmp72_;
	gboolean _tmp73_;
	gboolean _tmp74_;
	self = (ValaIntegerLiteral*) base;
	...
	if (_tmp39_ == 0) {
		...
		ValaProfile _tmp46_;
		gboolean _tmp47_;
		_tmp43_ = vala_code_context_get ();
		_tmp44_ = _tmp43_;
		_tmp45_ = vala_code_context_get_profile (_tmp44_);
		_tmp46_ = _tmp45_;
		_tmp47_ = _tmp46_ == VALA_PROFILE_DOVA;
		_vala_code_context_unref0 (_tmp44_);
		if (_tmp47_) {
			gboolean _tmp48_;
			_tmp48_ = u;
			if (_tmp48_) {
				...
				_tmp49_ = g_strdup ("uint64");
				_g_free0 (type_name);
				type_name = _tmp49_;
			} else {
				gchar* _tmp50_;
				vala_integer_literal_set_type_suffix (self, "L");
				_tmp50_ = g_strdup ("int64");
				_g_free0 (type_name);
				type_name = _tmp50_;
			}
		} else {
			...
			if (_tmp51_ == 1) {
				...
				if (_tmp55_) {
					...
					_tmp56_ = g_strdup ("uint64");
					_g_free0 (type_name);
					type_name = _tmp56_;
				} else {
					gchar* _tmp57_;
					vala_integer_literal_set_type_suffix (self, "LL");
					_tmp57_ = g_strdup ("int64");
					_g_free0 (type_name);
					type_name = _tmp57_;
				}
			}
	...
	_tmp60_ = _tmp59_;
	_tmp61_ = _tmp60_->root_symbol;
	_tmp62_ = vala_symbol_get_scope (_tmp61_);
	_tmp63_ = _tmp62_;
	_tmp64_ = type_name;
	_tmp65_ = vala_scope_lookup (_tmp63_, _tmp64_);
	st = VALA_STRUCT (_tmp65_);
	_tmp66_ = st;
	_tmp67_ = context;
	vala_code_node_check ((ValaCodeNode*) _tmp66_, _tmp67_);
	_tmp68_ = st;
	_tmp69_ = self->priv->_value;
	_tmp70_ = type_name;
	_tmp71_ = vala_integer_type_new (_tmp68_, _tmp69_, _tmp70_);
	_tmp72_ = _tmp71_;
	vala_expression_set_value_type ((ValaExpression*) self, (ValaDataType*) _tmp72_);
