...
ValaDelegate* vala_signal_get_delegate (ValaSignal* self, ValaDataType* sender_type, ValaCodeNode* node_reference) {
	...
	is_generic = FALSE;
	{
		...
		while (TRUE) {
			...
			_tmp42_ = _tmp41_;
			if (VALA_IS_GENERIC_TYPE (_tmp42_)) {
				is_generic = TRUE;
			}
	...
	if (_tmp43_) {
		...
		_tmp45_ = _tmp44_;
		_tmp46_ = _vala_code_node_ref0 (VALA_OBJECT_TYPE_SYMBOL (_tmp45_));
		cl = _tmp46_;
		{
		...
		}
		{
			...
			while (TRUE) {
				...
				_tmp80_ = _tmp79_;
				if (VALA_IS_GENERIC_TYPE (_tmp80_)) {
...
static gboolean vala_signal_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	if (_tmp27_) {
		...
		_tmp67_ = _tmp66_;
		_tmp68_ = _vala_code_node_ref0 (VALA_IS_OBJECT_TYPE_SYMBOL (_tmp67_) ? ((ValaObjectTypeSymbol*) _tmp67_) : NULL);
		cl = _tmp68_;
		_tmp69_ = cl;
