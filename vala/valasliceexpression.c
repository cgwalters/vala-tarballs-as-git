...
static gboolean vala_slice_expression_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	if (VALA_IS_ARRAY_TYPE (_tmp33_)) {
		...
		_tmp46_ = _tmp45_;
		if (VALA_IS_INTEGER_TYPE (_tmp46_)) {
			_tmp42_ = TRUE;
		} else {
			...
			_tmp50_ = _tmp49_;
			_tmp42_ = VALA_IS_ENUM_VALUE_TYPE (_tmp50_);
		}
		_tmp51_ = _tmp42_;
		if (!_tmp51_) {
		...
		_tmp60_ = _tmp59_;
		if (VALA_IS_INTEGER_TYPE (_tmp60_)) {
			_tmp56_ = TRUE;
		} else {
			...
			_tmp64_ = _tmp63_;
			_tmp56_ = VALA_IS_ENUM_VALUE_TYPE (_tmp64_);
		}
		_tmp65_ = _tmp56_;
		if (!_tmp65_) {
		...
		slice_method = VALA_IS_METHOD (_tmp74_) ? ((ValaMethod*) _tmp74_) : NULL;
