	...
	subsize = -1;
	{
		...
		while (TRUE) {
			...
			if (VALA_IS_INITIALIZER_LIST (_tmp30_)) {
				...
				_tmp39_ = rank;
				_tmp40_ = sl;
				_tmp41_ = vala_array_creation_expression_create_sizes_from_initializer_list (self, _tmp37_, VALA_INITIALIZER_LIST (_tmp38_), _tmp39_ - 1, _tmp40_);
				size = _tmp41_;
				_tmp42_ = size;
				if (_tmp42_ == (-1)) {
					result = -1;
	...
	if (_tmp48_ > 0) {
		{
			...
			while (TRUE) {
				...
				if (_tmp62_ == NULL) {
					...
					_tmp66_ = _tmp65_;
					if (VALA_IS_INTEGER_TYPE (_tmp66_)) {
						_tmp63_ = TRUE;
					} else {
						...
						_tmp69_ = _tmp68_;
						_tmp63_ = VALA_IS_ENUM_VALUE_TYPE (_tmp69_);
					}
					_tmp70_ = _tmp63_;
					if (!_tmp70_) {
