...
static gboolean vala_lock_statement_real_check (ValaCodeNode* base, ValaCodeContext* context) {
	...
	if (_tmp0_ != NULL) {
		...
		_tmp27_ = _tmp26_;
		_tmp28_ = _vala_code_node_ref0 (VALA_BLOCK (_tmp27_));
	...
	if (VALA_IS_MEMBER_ACCESS (_tmp41_)) {
		...
		_tmp44_ = _tmp43_;
		_tmp40_ = VALA_IS_LOCKABLE (_tmp44_);
	} else {
		_tmp40_ = FALSE;
	}
	_tmp45_ = _tmp40_;
...
" member");
	...
	_tmp59_ = _tmp58_;
	if (_tmp54_ != VALA_SYMBOL (_tmp59_)) {
	...
	_tmp66_ = _tmp65_;
	vala_lockable_set_lock_used (VALA_LOCKABLE (_tmp66_), TRUE);
